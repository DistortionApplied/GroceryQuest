...
export function TaskItem({
  ...
  return (
    <div className={`flex items-start gap-3 p-3 ${isHealthTask ? 'bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700' : 'bg-gray-50 dark:bg-gray-800'}`}>
      <input
        type="checkbox"
        checked={isChecked}
        onChange={handleToggle}
        disabled={isHealthTask}
        className={`mt-1 w-4 h-4 focus:ring-blue-500 dark:focus:ring-blue-400 ${isHealthTask ? 'text-amber-600 dark:text-amber-400 bg-amber-100 dark:bg-amber-900 border-amber-300 dark:border-amber-600 cursor-not-allowed' : 'text-blue-600 dark:text-blue-400 bg-gray-100 dark:bg-gray-700 border-gray-300 dark:border-gray-600'}`}
      />
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className={`px-2 py-0.5 text-xs ${isHealthTask ? 'bg-amber-200 dark:bg-amber-800 text-amber-800 dark:text-amber-200' : xpValue >= 0 ? 'bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200' : 'bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200'}`}>
            {xpValue >= 0 ? '+' : ''}{xpValue} XP
          </span>
          {isHealthTask && (
            <span className="px-2 py-0.5 bg-amber-200 dark:bg-amber-800 text-amber-800 dark:text-amber-200 text-xs font-medium">
              Auto
            </span>
          )}
        </div>
